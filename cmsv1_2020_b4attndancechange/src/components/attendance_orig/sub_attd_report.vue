<template>
  <div class="">
      ...
      <table class="table is-fullwidth is-bordered">
        ...
        <tr class="has-background-light has-text-weight-bold">
          ...
          <td v-for="(total,t1) in totalList" :key="t1">
            <button @click="openContentCovered(total.content)" class="button is-info">
    ...
    <div v-if="contentCovered" class="is-radiusless box">
        ...
        <p>{{contentCovered}}</p>
        <button @click="contentCovered=''" class="button is-danger is-small is-radiusless is-pulled-right">Close</button>
...
export default {
  ...
  methods: {
    ...
    reportTest(){
      ...
      this.$store.dispatch('attendanceStore/getAttdnReportBySubjectId',ob)
      .then(response=>{
          this.test1=response
        })
      .catch(error=>{
    ...
    getContentDt(){
      ...
      _.forEach(tempR,(v1,v2)=>{
          ...
          const vv=_.sortBy(v1,['csDateConducted'])
